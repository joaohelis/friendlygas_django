...
class OrionClient(object):
    ...
    @property
    def orion_query_url(self):
        return "%s/%s/queryContext" % (self.orion_host_prefix,self._orion_version_str)
        ...
        for key, val in d.iteritems():
            if key != '_about':
                if isinstance(val, int):
                    ...
                    attr_type = 'geo:point' if key == 'position' else 'string'
                    orion_attrs.append(dict(name=key, type=attr_type,
                                            value=val))
    ...
    def orion_version(self):
        ...
        headers = self.get_headers(exclude_content_type=True)
        r = requests.get(
            url,
            verify=self.verify,
            headers=headers,
    ...
    def query_entity(self, type_id, lat, longt, radius):
        '''query entities from geolocation within a radius
            fetch_entity(
                    entity_id='entity_id',
                    type_id='type_id',
                    attribute='attribute',
    ...
    def do_request(self,verb,url, headers, **kwds):
        if not(headers):
